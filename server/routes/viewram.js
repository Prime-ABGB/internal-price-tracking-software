...
router.get("/api", (req, res) => {
  db.all("SELECT * FROM ram_products", [], (err, rows) => {
    if (err) {
...
router.get("/product-comparison", (req, res) => {
  ...
  const query = `
    ...
    FROM ram_products p
    LEFT JOIN ram_product_links l ON p.product_sku = l.product_sku
    WHERE p.product_sku = ?;
  `;
...
async function scrapeOneRamRow(row) {
  ...
  const sql = `UPDATE ram_products SET prime_price = ?, mdcomp_price = ?, vedant_price = ?, pcstudio_price = ?, clarion_price = ?, ehubs_price = ?, prime_stock = ?, mdcomp_stock = ?, vedant_stock = ?, pcstudio_stock = ?, clarion_stock = ?, ehubs_stock = ? WHERE product_sku = ?;`;
  ...
  await new Promise((resolve, reject) => {
    db.run(sqlInsert, [product_sku, prime_price, prime_stock, mdcomp_price, mdcomp_stock, vedant_price, vedant_stock, pcstudio_price, pcstudio_stock, clarion_price, clarion_stock, ehubs_price, ehubs_stock, "ram"], (err) => (err ? reject(err) : resolve()));
  });
}
...
router.get("/webscraper", async (req, res) => {
  try {
    const rows = await new Promise((resolve, reject) => {
      db.all("SELECT * FROM ram_product_links", (err, rows) => (err ? reject(err) : resolve(rows || [])));
    });
    let filtered = req.query.sku ? rows.filter((r) => r.product_sku === req.query.sku) : rows;
    await setupBrowser();
    for (const row of filtered) {
      try {
        await scrapeOneRamRow(row);
      } catch (error) {
...
router.post("/products", async (req, res) => {
  try {
    ...
    await new Promise((resolve, reject) => {
      db.run("INSERT OR REPLACE INTO ram_products (product_name, product_sku, prime_price) VALUES (?, ?, ?)", [product_name, product_sku, prime_price], (err) => (err ? reject(err) : resolve()));
    });
    await new Promise((resolve, reject) => {
      db.run(
        "INSERT OR REPLACE INTO ram_product_links (product_sku, prime_link, mdcomp_link, vedant_link, pcstudio_link, clarion_link, ehubs_link) VALUES (?, ?, ?, ?, ?, ?, ?)",
