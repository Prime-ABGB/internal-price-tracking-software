...
router.get("/api", (req, res) => {
  db.all("SELECT * FROM gpu_products", [], (err, rows) => {
    if (err) {
...
router.get("/product-comparison", (req, res) => {
  ...
  const query = `
    ...
    FROM gpu_products p
    LEFT JOIN gpu_product_links l ON p.product_sku = l.product_sku
    WHERE p.product_sku = ?;
  `;
...
async function scrapeOneGpuRow(row) {
  const {
    product_sku,
    prime_link,
    mdcomp_link,
  ...
  const sql = `
    UPDATE gpu_products
    SET prime_price = ?, mdcomp_price = ?, vedant_price = ?, pcstudio_price = ?,
        clarion_price = ?, ehubs_price = ?,
        prime_stock = ?, mdcomp_stock = ?, vedant_stock = ?, pcstudio_stock = ?,
        clarion_stock = ?, ehubs_stock = ?
    WHERE product_sku = ?;
  `;
  ...
  const sqlInsert = `
    INSERT INTO historical_prices (sku, date, prime_price, prime_stock, mdcomp_price, mdcomp_stock, vedant_price, vedant_stock, pcstudio_price, pcstudio_stock, clarion_price, clarion_stock, ehubs_price, ehubs_stock, category)
    VALUES (?, DATE('now'), ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ON CONFLICT(sku, date) DO UPDATE SET prime_price = excluded.prime_price, prime_stock = excluded.prime_stock, mdcomp_price = excluded.mdcomp_price, mdcomp_stock = excluded.mdcomp_stock, vedant_price = excluded.vedant_price, vedant_stock = excluded.vedant_stock, pcstudio_price = excluded.pcstudio_price, pcstudio_stock = excluded.pcstudio_stock, clarion_price = excluded.clarion_price, clarion_stock = excluded.clarion_stock, ehubs_price = excluded.ehubs_price, ehubs_stock = excluded.ehubs_stock, category = excluded.category;
  `;
  const insertParams = [product_sku, prime_price, prime_stock, mdcomp_price, mdcomp_stock, vedant_price, vedant_stock, pcstudio_price, pcstudio_stock, clarion_price, clarion_stock, ehubs_price, ehubs_stock, "gpu"];
  await new Promise((resolve, reject) => {
    db.run(sqlInsert, insertParams, (err) => (err ? reject(err) : resolve()));
  });
}
...
router.get("/webscraper", async (req, res) => {
  try {
    const rows = await new Promise((resolve, reject) => {
      db.all("SELECT * FROM gpu_product_links", (err, rows) => {
        if (err) reject(err);
        else resolve(rows || []);
      });
    });
    const filtered = req.query.sku ? rows.filter((r) => r.product_sku === req.query.sku) : rows;
    await setupBrowser();
    for (const row of filtered) {
      try {
        await scrapeOneGpuRow(row);
      } catch (error) {
...
router.post("/products", async (req, res) => {
  try {
    ...
    await new Promise((resolve, reject) => {
      db.run("INSERT OR REPLACE INTO gpu_products (product_name, product_sku, prime_price) VALUES (?, ?, ?)", [product_name, product_sku, prime_price], (err) => (err ? reject(err) : resolve()));
    });
    await new Promise((resolve, reject) => {
      db.run(
        "INSERT OR REPLACE INTO gpu_product_links (product_sku, prime_link, mdcomp_link, vedant_link, pcstudio_link, clarion_link, ehubs_link) VALUES (?, ?, ?, ?, ?, ?, ?)",
