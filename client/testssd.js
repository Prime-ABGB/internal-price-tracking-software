async function fetchData() {
  ...
  try {
    const response = await fetch("/ssd/api");
  ...
  respo = respo.map((obj) => {
    ...
    if (obj.mdcomp_stock === "In Stock" && obj.mdcomp_price !== null) {
      minPrice = Math.min(minPrice, obj.mdcomp_price);
    }
    if (obj.vedant_stock === "In Stock" && obj.vedant_price !== null) {
      minPrice = Math.min(minPrice, obj.vedant_price);
    }
    if (obj.pcstudio_stock === "In Stock" && obj.pcstudio_price !== null) {
      minPrice = Math.min(minPrice, obj.pcstudio_price);
    }
    if (obj.clarion_stock === "In Stock" && obj.clarion_price !== null) {
      minPrice = Math.min(minPrice, obj.clarion_price);
    }
    if (obj.ehubs_stock === "In Stock" && obj.ehubs_price !== null) {
...
function openPriceComparison(productSku) {
  fetch(`/ssd/product-comparison?product=${encodeURIComponent(productSku)}`)
