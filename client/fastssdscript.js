async function fetchData() {
  ...
  try {
    const response = await fetch("fastmoving/ssd/api");
  ...
  respo = respo.map((obj) => {
    ...
    if (obj.ossd_stock === "In Stock" && obj.ossd_price !== null && obj.ossd_price !== 0) {
      minPrice = Math.min(minPrice, obj.ossd_price);
    }
...
function openPriceComparison(productSku) {
  fetch(`fastmoving/ssd/product-comparison?product=${encodeURIComponent(productSku)}`)
    .then((response) => response.json())
    .then((product) => {
      localStorage.setItem("productComparisonData", JSON.stringify(product));
      return fetch(`/graphfastmoving?product=${encodeURIComponent(productSku)}`);
    })
    .then((response) => response.json())
    .then((chartData) => {
      localStorage.setItem("chartData", JSON.stringify(chartData));
      return fetch(`/analytics?product=${encodeURIComponent(productSku)}`); 
    })
    .then((response) => response.json())
    .then((analyticsData) => {
      localStorage.setItem("analyticsData", JSON.stringify(analyticsData));
      window.open("/fastproductcompare.html", "_blank");
    })
    .catch((error) => {
