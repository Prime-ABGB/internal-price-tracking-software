...
export default function RAMPage() {
  const { products: ramProducts, loading, error } = useProducts('ram')
  const [searchQuery, setSearchQuery] = useState('')
  const [priceRange] = useState({ min: 0, max: 200000 })

  const filteredProducts = useMemo(() => {
    return ramProducts.filter((product) => {
      const matchesSearch =
  ...
  }, [ramProducts, searchQuery, priceRange])
    ...
    <>
      ...
      <main className="min-h-screen bg-background">
        <div className="container py-8 sm:py-12">
          <div className="mb-8">
            <h1 className="text-3xl font-bold">RAM Price Tracker</h1>
            <p className="mt-2 text-muted-foreground">
              Compare memory prices and specifications
            </p>
          </div>
          ...
          {loading && (
            <div className="flex items-center justify-center py-12">
              <div className="text-center">
                ...
                <p className="text-muted-foreground">Loading RAM data...</p>
              </div>
            </div>
          )}
