...
export default function GPUPage() {
  const { products: gpuProducts, loading, error } = useProducts('gpu')
  const [searchQuery, setSearchQuery] = useState('')
  const [priceRange] = useState({ min: 0, max: 200000 })

  const filteredProducts = useMemo(() => {
    return gpuProducts.filter((product) => {
      const matchesSearch =
  ...
  }, [gpuProducts, searchQuery, priceRange])
        ...
        <div className="container py-8 sm:py-12">
          <div className="mb-8">
            ...
            <p className="mt-2 text-muted-foreground">
              Compare graphics card prices and specifications
            </p>
          </div>
          ...
          {loading && (
            <div className="flex items-center justify-center py-12">
              <div className="text-center">
                ...
                <p className="text-muted-foreground">Loading GPU data...</p>
              </div>
            </div>
          )}
